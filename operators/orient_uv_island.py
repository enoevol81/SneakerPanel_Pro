...
class OBJECT_OT_OrientUVIsland(Operator):
    ...
    def poll(cls, context):
        ...
        return (
            ...
            and bpy.data.objects.get("Toe_Marker") is not None
            and bpy.data.objects.get("Toe_Direction_Marker") is not None
        )
    ...
    def execute(self, context):
        ...
        if not toe_marker:
            self.report(
                {"ERROR"},
                "Toe_Marker not found. Please use 'Define Toe Direction' first.",
            )
            return {"CANCELLED"}
        ...
        if not direction_marker:
            self.report(
                {"ERROR"},
                "Toe_Direction_Marker not found. Please complete 'Define Toe Direction'.",
            )
            return {"CANCELLED"}
        ...
        try:
            ...
            toe_vertex_index = min(world_coords, key=lambda x: (x[1] - toe_pos).length)[
                0
            ]
            direction_vertex_index = min(
