...
class MESH_OT_SimpleGridFill(Operator):
    ...
    def execute(self, context):
        ...
        try:
            ...
            self.report({'INFO'}, f"Starting: {len(bm.verts)} verts, {len(bm.edges)} edges, {len(bm.faces)} faces")
            ...
            if success and self.smooth_after:
                try:
                    ...
                    if interior_verts:
                        self.report({'INFO'}, f"Smoothing {len(interior_verts)} interior vertices, preserving {len(boundary_verts)} boundary vertices")
                        ...
                        for iteration in range(self.smooth_iterations):
                            ...
                            for vert in interior_verts:
                                ...
                                if connected_positions:
                                    avg_pos = sum(connected_positions, vert.co * 0) / len(connected_positions)
                        ...
                        bmesh.update_edit_mesh(obj.data)
                        obj.data.update()
                        self.report({'INFO'}, f"Applied {self.smooth_iterations} smoothing iterations")
                    else:
                        self.report({'INFO'}, "No interior vertices to smooth - all vertices are on boundary")
                        
                except Exception as e:
                    self.report({'WARNING'}, f"Boundary-preserving smoothing failed: {str(e)}")
            
            # Restore original selection mode
            bpy.ops.mesh.select_mode(
                type='VERT' if select_mode[0] else 
                'EDGE' if select_mode[1] else 
                'FACE'
            )
