...
class SPP_OT_apply_lace(Operator):
    ...
    scale: FloatProperty(
        name="Scale", description="Lace width scaling", default=1.0, min=0.01, max=10.0
    )

    tilt: FloatProperty(
        name="Tilt", description="Tilt angle along curve", default=0.0, subtype="ANGLE"
    )
    ...
    flip_v: BoolProperty(
        name="Flip V", description="Flip UV V coordinate", default=False
    )

    flip_normal: BoolProperty(
        name="Flip Normal", description="Flip face normals", default=False
    )

    shade_smooth: BoolProperty(
        name="Shade Smooth", description="Apply smooth shading", default=True
    )
    ...
    custom_profile: bpy.props.StringProperty(
        name="Custom Profile", description="Name of custom profile object", default=""
    )
    ...
    def execute(self, context):
        ...
        if lace_type == "CUSTOM":
            ...
            if scene.spp_lace_custom_profile == target_curve:
                self.report(
                    {"ERROR"},
                    "Custom profile cannot be the same as the target curve. Please select a different curve as the custom profile.",
                )
                return {"CANCELLED"}
        ...
        if not node_group:
            self.report(
                {"ERROR"},
                f"Node group '{node_group_name}' not found. Make sure lace assets are loaded.",
            )
            return {"CANCELLED"}
        ...
        for modifier in target_curve.modifiers:
            if modifier.type == "NODES" and modifier.node_group:
                ...
                if any(
                    lace_name in modifier.node_group.name
                    for lace_name in [
                        "spp_lace_round",
                        "spp_lace_oval",
                        "spp_lace_flat",
        ...
        try:
            ...
            if "Scale" in modifier:
                modifier["Scale"] = (
                    scene.spp_lace_scale if hasattr(scene, "spp_lace_scale") else 0.05
                )

            if "Resample" in modifier:
                modifier["Resample"] = (
                    scene.spp_lace_resample
            ...
            if "Tilt" in modifier:
                modifier["Tilt"] = (
                    scene.spp_lace_tilt if hasattr(scene, "spp_lace_tilt") else 0.0
                )
            ...
            # the modifier.
            if (
                "Socket_10" in modifier
                and hasattr(scene, "spp_lace_free_normal")
                and scene.spp_lace_normal_mode == "2"
            ):
