...
def lacefromcurves_node_group():
    ...
    #Socket Resample 
    resample__socket = lacefromcurves.interface.new_socket(name = "Resample ", in_out='INPUT', socket_type = 'NodeSocketInt')
    ...
    #Socket Material
    material_socket = lacefromcurves.interface.new_socket(name = "Material", in_out='INPUT', socket_type = 'NodeSocketMaterial')
...
class OBJECT_OT_add_lace_nodegroup(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if not node_group:
            self.report({'ERROR'}, "LaceFromCurves node group not found or failed to create")
            return {'CANCELLED'}
