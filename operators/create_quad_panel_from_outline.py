...
class MESH_OT_CreateQuadPanelFromOutline(Operator):
    ...
    def execute(self, context):
        ...
        if not (original_outline_obj and original_outline_obj.type == "MESH"):
            self.report({"ERROR"}, "An active Mesh outline object is required.")
            return {"CANCELLED"}
        ...
        try:
            ...
            bpy.ops.mesh.inset(
                thickness=self.inset_thickness, use_even_offset=True, depth=0
            )
            self.report(
                {"INFO"}, f"Applied inset with thickness {self.inset_thickness}."
            )
            ...
            if working_obj.name in bpy.data.objects:
        ...
        context.view_layer.objects.active = working_obj
        self.report(
            {"INFO"}, f"Successfully created quad panel border: '{working_obj.name}'."
        )
        return {"FINISHED"}
