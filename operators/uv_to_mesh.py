...
class OBJECT_OT_UVToMesh(Operator):
    ...
    def execute(self, context):
        ...
        if selected_objs_backup: bpy.ops.object.select_all(action='DESELECT')
        ...
        # ...
        area_3d = 0; bm = bmesh.new(); polygons_to_process = me0_from_conversion.polygons
        if not polygons_to_process: # ... (error handling) ...
            ...
            return {'CANCELLED'}
        active_uv_layer_data = me0_from_conversion.uv_layers.active.data; uv_to_bm_vert_map = {}
        for face in polygons_to_process:
            area_3d += face.area; face_bm_verts = []
            for loop_idx in face.loop_indices:
                uv = active_uv_layer_data[loop_idx].uv; uv_key = (round(uv.x,6),round(uv.y,6))
                if uv_key not in uv_to_bm_vert_map: uv_to_bm_vert_map[uv_key] = bm.verts.new((uv.x,uv.y,0.0))
                face_bm_verts.append(uv_to_bm_vert_map[uv_key])
            if len(face_bm_verts)>=3:
                try: 
                    new_face = bm.faces.new(face_bm_verts)
                    if self.materials: new_face.material_index = face.material_index
                except ValueError: pass
        if not bm.faces: # ... (error handling) ...
             self.report({'ERROR'}, "No valid UV faces generated."); bpy.data.objects.remove(ob0_for_conversion); bpy.data.meshes.remove(me0_from_conversion); bm.free(); return {'CANCELLED'}
        ...
        if self.auto_scale:
            ...
            if area_uv_unscaled > 1e-9:
                ...
                context.view_layer.objects.active = ob_uv; ob_uv.select_set(True)
                ...
                if current_active and current_active.name in bpy.data.objects: context.view_layer.objects.active = current_active
                else: context.view_layer.objects.active = ob_uv # Fallback if original active is gone

            elif area_3d > 1e-9: self.report({'WARNING'}, "UV mesh area zero, cannot auto-scale.")
        ...
        for o_sel in prev_selected: 
            if o_sel and o_sel.name in bpy.data.objects and o_sel != ob_uv : o_sel.select_set(True)
        if prev_active and prev_active.name in bpy.data.objects and prev_active != ob_uv: context.view_layer.objects.active = prev_active
        else: context.view_layer.objects.active = ob_uv
        ...
        gp_location = ob_uv.location # ob_uv is at origin if scale was applied without location change
        
        # Create GP object
        bpy.ops.object.select_all(action='DESELECT') # Deselect all before adding new GP
        bpy.ops.object.grease_pencil_add(location=gp_location, type='EMPTY') # Add blank GP
        gp_obj_uv_draw = context.active_object
        
        # Name the GP object
        gp_name_base = f"{panel_name_prop}_GPDraw_UV_{panel_count}" if panel_name_prop and panel_name_prop.strip() else f"GPDraw_UV_{panel_count}"
        ...
        self.report({'INFO'}, f"Grease Pencil '{gp_obj_uv_draw.name}' added to collection.")

        # Configure Grease Pencil for drawing on surface (similar to add_gp_draw.py)
        context.view_layer.objects.active = gp_obj_uv_draw # Make GP active for mode set
        ...
        if brush:
            brush.use_smooth_stroke = getattr(context.scene, "spp_use_stabilizer", False)
            brush.smooth_stroke_factor = getattr(context.scene, "spp_stabilizer_factor", 0.75)
            brush.smooth_stroke_radius = getattr(context.scene, "spp_stabilizer_radius", 35)
        ...
        area_3d = None; region_3d_window = None
        ...
        if area_3d and region_3d_window:
            ...
            if space_data.type == 'VIEW_3D':
                original_perspective = space_data.region_3d.view_perspective # Store for potential restore
