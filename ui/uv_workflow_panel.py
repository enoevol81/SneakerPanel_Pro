...
class OBJECT_PT_UVWorkflow(Panel):
    ...
    def draw(self, context):
        ...
        tools = curve_box.box(); tools.label(text="Curve Editing Tools", icon="TOOL_SETTINGS")
        ...
        opts = tools.column(align=True); opts.label(text="Curve Options:", icon="CURVE_DATA")
        cyclic = opts.row(); cyclic.prop(S, "spp_curve_cyclic", text=""); cyclic.label(text="Cyclic Curve")
        mir = tools.column(align=True); mh = mir.row(align=True)
        ...
        if hasattr(S, "spp_sampler_fidelity"):
            col_sample = box_sample.column(align=True); col_sample.prop(S, "spp_sampler_fidelity", text="Boundary Samples")
        ...
        boundary_header = box_boundary.row(); boundary_header.label(text="Step 4: Boundary Check", icon='CHECKMARK')
        ...
        # Step 5
        box_fill = uv_box.box(); box_fill.label(text="Step 5: Fill Border with Grid", icon='GRID')
        ...
        # Step 6
        box_proj = uv_box.box(); box_proj.label(text="Step 6:  Project To Surface", icon='OUTLINER_OB_MESH')
