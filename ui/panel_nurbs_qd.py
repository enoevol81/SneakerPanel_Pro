...
class SPP_PT_NurbsToSurface(Panel):
    ...
    def draw(self, context):
        ...
        b = row.operator("wm.context_set_enum", text="Bezier Q&D",
                         depress=(sc.spp_nurbs_qd_active=='QD_BEZIER'),
                         icon='CURVE_BEZCURVE')
        b.data_path = "scene.spp_nurbs_qd_active"; b.value = 'QD_BEZIER'
        b = row.operator("wm.context_set_enum", text="UV Curve Q&D",
    ...
    def draw_qd_bezier(self, layout, context):
        ...
        gp_header.label(text="Step 1: Create Grease Pencil - Bezier Q&D", icon="GREASEPENCIL")
        ...
        mirror.operator("curve.mirror_selected_points_at_cursor", text="Mirror at Cursor", icon="CURVE_BEZCIRCLE")

        # Step 3: Generate NURBS Surface (Q&D)
        surf_box = layout.box()
        surf_box.label(text="Step 3: Generate NURBS Surface (Q&D)", icon='SURFACE_NSURFACE')
        op_bs = surf_box.operator("spp.convert_bezier_to_surface", text="Convert Bezier to Surface", icon='SURFACE_DATA')
        ...
        mesh_box.operator("object.convert", text="Convert to Mesh Object", icon='MESH_GRID').target = 'MESH'
    ...
    def draw_qd_uvcurve(self, layout, context):
        ...
        row = box.row(); row.operator("object.uv_to_mesh", icon='MESH_DATA')
        ...
        if context.active_object and context.active_object.type == 'MESH':
            ...
            if has_violations:
                ...
                reselect_row.operator("mesh.reselect_uv_violations", text="Step 2a: UV Boundary Check - Re-select Violations", icon='RESTRICT_SELECT_OFF')
